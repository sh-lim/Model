//CMS ridge paper (PYTHIA pp 13 TeV, multiplicity bin 1<pT<2 GeV/c)
...
void make_hist_pp13TeV_06(const char *fname="file.lst"){
	...
	while ( flist >> ffname ){
		...
		T->SetBranchAddress("p_id",part_pid);
		T->SetBranchAddress("p_eta",part_eta);
		T->SetBranchAddress("p_phi",part_phi);
		T->SetBranchAddress("p_pt",part_pt);
		...
		for (int ien=0; ien<nentries; ien++){
			...
			for (int ip=0; ip<npart; ip++){
				if ( fabs(part_eta[ip])<2.4 && part_pt[ip]>0.4 ){
					nmult_mid++;
				}
