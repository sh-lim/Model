...
void Draw_alice_yield(){
	...
	TFile *infile = new TFile("../ALICE-kinematics/outfile_hist_pp13TeV_set00_grp001_try002.root","read");
	...
	for (int imult=0; imult<nmult; imult++){
		...
		for (int ipt=0; ipt<npt; ipt++){
			...
			h2d_same[imult][ipt]->RebinX(6);
			h2d_mixed[imult][ipt]->RebinX(6);
			...
			int etabin_min = h2d_same[imult][ipt]->GetYaxis()->FindBin(-1.8+0.001);
			int etabin_max = h2d_same[imult][ipt]->GetYaxis()->FindBin(-1.6-0.001);
			...
			etabin_min = h2d_same[imult][ipt]->GetYaxis()->FindBin(+1.6+0.001);
			etabin_max = h2d_same[imult][ipt]->GetYaxis()->FindBin(+1.8-0.001);
			...
			//normalization
			h2d_same[imult][ipt]->RebinY(2);
			h2d_mixed[imult][ipt]->RebinY(2);
			...
			f1d_dphi[imult][ipt] = new TF1("f1","[0]*( 1 + 2*[1]*cos(x) + 2*[2]*cos(2*x))",-const_pi/2,1.2*const_pi/2);
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			h2d_same[imult][ipt]->SetAxisRange(-1.8+0.001,+1.8-0.001,"Y");

			float ymax = h2d_same[imult][ipt]->GetMaximum();
			//float ymin = h2d_same[imult][ipt]->GetMinimum();
			float ymin = h2d_same[imult][ipt]->GetBinContent(h2d_same[imult][ipt]->FindBin(-TMath::Pi()/2,0));
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			SetHistoStyle("|#Delta#phi|","#frac{1}{N_{trig}}#frac{dN^{pair}}{d#Delta#phi}","",14,12);
	...
	for (int imult=0; imult<nmult; imult++){
		for (int ipt=0; ipt<npt; ipt++){
			...
			SetHistoStyle("|#Delta#phi|","#frac{1}{N_{trig}}#frac{dN^{pair}}{d#Delta#phi} - C_{ZYAM}","",14,12);
			htmp->GetYaxis()->SetTitleOffset(7.0);
			htmp->GetXaxis()->SetTitleOffset(5.0);
