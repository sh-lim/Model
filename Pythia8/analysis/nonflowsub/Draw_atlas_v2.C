...
void Draw_atlas_v2(const char *dataset="pp13TeV_set20_grp008_try101", const bool bSAVE=true){
  ...
  const char *system = "p+p";
	...
	TH1D *hntrig = (TH1D*)infile->Get("hntrig_f");
	TH1D *hntrig_mixed = (TH1D*)infile->Get("hntrig_mixed_f");

	for (int im=0; im<nmult; im++){
		h2d_same_dphi_deta[im] = (TH2D*)infile->Get(Form("h2d_same_dphi_deta_f_m%02d",im));
		h2d_mixed_dphi_deta[im] = (TH2D*)infile->Get(Form("h2d_mixed_dphi_deta_f_m%02d",im));

    //h2d_same_dphi_deta[im]->RebinX(3);
    //h2d_mixed_dphi_deta[im]->RebinX(3);
    h2d_same_dphi_deta[im]->RebinY(10);
    h2d_mixed_dphi_deta[im]->RebinY(10);
  ...
  //return;
	...
	TH1D *hv33_dir_nref_1 = new TH1D("hv33_dir_nref_1","",20,0,200);
	TH1D *hv33_dir_nref_2 = new TH1D("hv33_dir_nref_2","",20,0,200);
	TH1D *hv33_dir_nref_3 = new TH1D("hv33_dir_nref_3","",20,0,200);
	...
	TGraphErrors *gv33_sub_nref_1 = new TGraphErrors;
	TGraphErrors *gv33_sub_nref_2 = new TGraphErrors;
	TGraphErrors *gv33_sub_nref_3 = new TGraphErrors;
  ...
  int g_count = 0;
	...
	TCanvas *c1_1 = new TCanvas("c1_1","c1_1",1.1*250*4,250*4);
	c1_1->Divide(4,4);
    ...
    htmp = (TH1D*)gPad->DrawFrame(-const_pi/2, min-0.05*(max-min), 3*const_pi/2, max+0.05*(max-min));
		SetHistoStyle("#Delta#phi (rad)","Per trigger yield","",14,12);
		...
		hv33_dir_nref_1->SetBinContent(im+1, fdir_nref_1[im]->GetParameter(3));
		hv33_dir_nref_1->SetBinError(im+1, fdir_nref_1[im]->GetParError(3));

    if ( im>nref_1+1 ){

      g_h_central = h1d_same_dphi_long[im][0];
      g_h_peripheral = (TH1D*)h1d_same_dphi_long[nref_1][0]->Clone(Form("g_h_peripheral_nref_1_mult%d",im));
      ...
      h_rescaledperipheral_nref_1[im]->Draw("p same");
			...
			float v33 = f_combined_nref_1[im]->GetParameter(2);
			float v33_err = f_combined_nref_1[im]->GetParError(2);

      gv22_sub_nref_1->SetPoint(g_count, 10*im+5, v22);
      gv22_sub_nref_1->SetPointError(g_count, 0, 0);

      gv33_sub_nref_1->SetPoint(g_count, 10*im+5, v33);
      gv33_sub_nref_1->SetPointError(g_count, 0, 0);
  ...
  g_count = 0;
	TCanvas *c1_2 = new TCanvas("c1_2","c1_2",1.1*250*4,250*4);
	c1_2->Divide(4,4);
    ...
    htmp = (TH1D*)gPad->DrawFrame(-const_pi/2, min-0.05*(max-min), 3*const_pi/2, max+0.05*(max-min));
		...
		hv33_dir_nref_2->SetBinContent(im+1, fdir_nref_2[im]->GetParameter(3));
		hv33_dir_nref_2->SetBinError(im+1, fdir_nref_2[im]->GetParError(3));

    if ( im>nref_2 ){

      g_h_central = h1d_same_dphi_long[im][0];
			g_h_peripheral = (TH1D*)h1d_same_dphi_long[nref_2][0]->Clone(Form("g_h_peripheral_nref_2_mult%d",im));
      ...
      h_rescaledperipheral_nref_2[im]->Draw("p same");
			...
			float v33 = f_combined_nref_2[im]->GetParameter(2);
			float v33_err = f_combined_nref_2[im]->GetParError(2);

      gv22_sub_nref_2->SetPoint(g_count, 10*im+5, v22);
      gv22_sub_nref_2->SetPointError(g_count, 0, 0);

      gv33_sub_nref_2->SetPoint(g_count, 10*im+5, v33);
      gv33_sub_nref_2->SetPointError(g_count, 0, 0);
  ...
  g_count = 0;
	TCanvas *c1_3 = new TCanvas("c1_3","c1_3",1.1*250*4,250*4);
	c1_3->Divide(4,4);
    ...
    htmp = (TH1D*)gPad->DrawFrame(-const_pi/2, min-0.05*(max-min), 3*const_pi/2, max+0.05*(max-min));
		...
		hv33_dir_nref_3->SetBinContent(im+1, fdir_nref_3[im]->GetParameter(3));
		hv33_dir_nref_3->SetBinError(im+1, fdir_nref_3[im]->GetParError(3));

    if ( im>nref_3 ){

      g_h_central = h1d_same_dphi_long[im][0];
			g_h_peripheral = (TH1D*)h1d_same_dphi_long[nref_3][0]->Clone(Form("g_h_peripheral_nref_3_mult%d",im));
			g_h_peripheral->Add(h1d_same_dphi_long[0][0]);
      ...
      //
			...
			h_rescaledperipheral_nref_3[im]->Add(h1d_same_dphi_long[0][0]);
			...
			h_nref_3[im] = (TH1D*)h_rescaledperipheral_nref_3[im]->Clone(Form("h_nref_3_mult%d",im));
			h_nref_3[im]->Reset();
			for (int ibin=0; ibin<h_nref_3[im]->GetNbinsX(); ibin++){
				float xx = h_nref_3[im]->GetBinCenter(ibin+1);
				float yy = func_nref_3[im]->Eval(xx);
				h_nref_3[im]->SetBinContent(ibin+1, yy);
			}
      ...
      h_rescaledperipheral_nref_3[im]->Draw("p same");
			...
			float v33 = f_combined_nref_3[im]->GetParameter(2);
			float v33_err = f_combined_nref_3[im]->GetParError(2);

      gv22_sub_nref_3->SetPoint(g_count, 10*im+5, v22);
      gv22_sub_nref_3->SetPointError(g_count, 0, 0);

      gv33_sub_nref_3->SetPoint(g_count, 10*im+5, v33);
      gv33_sub_nref_3->SetPointError(g_count, 0, 0);
  ...
  c1_3->Update();
	...
	TCanvas *c2 = new TCanvas("c2","c2",1.3*500,500);
	SetPadStyle();
	gPad->SetBottomMargin(0.12);
	gPad->SetLeftMargin(0.13);

	{
		float max = h1d_same_dphi_long[10][0]->GetMaximum();
		float min = h1d_same_dphi_long[10][0]->GetMinimum();

    htmp = (TH1D*)gPad->DrawFrame(-const_pi/2, min-0.1*(max-min), 3*const_pi/2, max+0.1*(max-min));
		SetHistoStyle("#Delta#varphi (rad)","Y(#Delta#varphi)","",24,20);
		htmp->GetYaxis()->SetTitleOffset(1.2);
		h1d_same_dphi_long[10][0]->SetMarkerStyle(20);
		h1d_same_dphi_long[10][0]->Draw("p same");

		h_rescaledperipheral_nref_3[10]->SetLineWidth(1);
		h_rescaledperipheral_nref_3[10]->SetLineColor(1);
		h_rescaledperipheral_nref_3[10]->SetMarkerStyle(24);
		h_rescaledperipheral_nref_3[10]->Draw("p same");

		f_ped_nref_3[10]->Draw("same");
		//func_nref_3[8]->Draw("same");
		f_combined_nref_3[10]->Draw("same");
		h_nref_3[10]->SetLineColor(2);
		h_nref_3[10]->Draw("same");

    {
      TLegend *leg = new TLegend(0.15,0.43,0.5,0.93);
      leg->SetFillStyle(0);
			leg->SetBorderSize(0);
			leg->SetTextFont(43);
			leg->SetTextSize(18);
      ...
      leg->AddEntry("","100#leqN_{ch}<110","h");
      leg->AddEntry("","2<|#Delta#eta|<5, 0.5<p_{T}<5 GeV/c","h");
			leg->AddEntry(h1d_same_dphi_long[10][0],"Y^{HM}(#Delta#varphi)","P");
			leg->AddEntry(h_rescaledperipheral_nref_3[10],"FY^{LM}(#Delta#varphi)+G","P");
			//leg->AddEntry(h_nref_3[10],"Template fit","L");
			leg->AddEntry(h_nref_3[10],"Y^{template}(#Delta#varphi)","L");
			leg->AddEntry(f_combined_nref_3[10],"Y^{Ridge}(#Delta#varphi)+FY^{LM}(0)","L");
			leg->AddEntry(f_ped_nref_3[10],"G+FY^{LM}(0)","L");
			leg->AddEntry("","","");
			leg->AddEntry("",Form("v_{2,2}=%4.5f#pm%4.5f",f_combined_nref_3[10]->GetParameter(1), f_combined_nref_3[10]->GetParError(1)),"h");
      leg->Draw();
    }
	}
	...
	SetHistoStyle("N_{ch}","v_{22}","",24,24);
	...
	{
		TBox *box = new TBox(0,0.064*0.064,150,0.065*0.065);
		box->SetFillStyle(1001);
		box->SetFillColorAlpha(9,0.5);
		box->Draw("");
	}
	...
	{
		TLegend *leg = new TLegend(0.45,0.50,0.85,0.95);
		leg->SetFillStyle(0);
		leg->SetBorderSize(0);
	...
	htmp = (TH1D*)gPad->DrawFrame(0,-0.003,150,0.003);
	SetHistoStyle("N_{ch}","v_{33}","",24,24);
	htmp->GetXaxis()->SetTitleOffset(1.0);
	htmp->GetYaxis()->SetTitleOffset(1.5);

	{
		TBox *box = new TBox(0,-0.01*0.01,150,0.01*0.01);
		box->SetFillStyle(1001);
		box->SetFillColorAlpha(11,0.5);
		box->Draw("");
	}
	...
	hv33_dir_nref_1->SetMarkerStyle(24);
	hv33_dir_nref_1->SetMarkerColor(1);
	hv33_dir_nref_1->SetLineColor(1);
	hv33_dir_nref_1->SetLineWidth(2);
	hv33_dir_nref_1->Draw("p same");

	gv33_sub_nref_1->SetLineWidth(4);
	gv33_sub_nref_1->SetLineColor(1);
	gv33_sub_nref_1->Draw("c same");

	gv33_sub_nref_2->SetLineWidth(4);
	gv33_sub_nref_2->SetLineColor(2);
	gv33_sub_nref_2->Draw("c same");

	gv33_sub_nref_3->SetLineWidth(4);
	gv33_sub_nref_3->SetLineColor(4);
	gv33_sub_nref_3->Draw("c same");

	if ( bSAVE ){
		TFile *outfile = new TFile(Form("outfile_v22_mult_%s.root",dataset),"recreate");
		gv22_sub_nref_1->Write("gv22_sub_nref_1");
		gv22_sub_nref_2->Write("gv22_sub_nref_2");
		gv22_sub_nref_3->Write("gv22_sub_nref_3");
		outfile->Close();

		outfile = new TFile(Form("outfile_v33_mult_%s.root",dataset),"recreate");
		gv33_sub_nref_1->Write("gv33_sub_nref_1");
		gv33_sub_nref_2->Write("gv33_sub_nref_2");
		gv33_sub_nref_3->Write("gv33_sub_nref_3");
		outfile->Close();

		/*
		outfile = new TFile(Form("outfile_template_fit_%s.root",dataset),"recreate");

		for (int im=8; im<9; im++){
			h1d_same_dphi_long[im][0]->Write(Form("hHM_mult%d",im));
			h_rescaledperipheral_nref_3[im]->Write(Form("hLM_mult%d",im));
			f_ped_nref_3[im]->Write(Form("fPED_mult%d",im));
			f_combined_nref_3[im]->Write(Form("fFLOW_mult%d",im));
			h_nref_3[im]->Write(Form("hFIT_mult%d",im));
		}
		*/
