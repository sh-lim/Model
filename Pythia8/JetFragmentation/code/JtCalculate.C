...
void JtCalculate(){
    ...
    double z_edge[8] = {0,0.05,0.1,0.2,0.4,0.6,0.8,1};
	...
	TH1D *sigz = new TH1D("sigz", "sigz", 7,z_edge);
	TH1D *incljt = new TH1D("incljt", "incljt", 27, j_edge);
	TH1D *inclz = new TH1D("inclz", "inclz", 7,z_edge);

	TH1D *perp_sigjt = new TH1D("perp_sigjt", "perp_sigjt", 27, j_edge);
	TH1D *perp_sigz = new TH1D("perp_sigz", "perp_sigz", 7,z_edge);
	TH1D *perp_incljt = new TH1D("perp_incljt", "perp_incljt", 27, j_edge);
	TH1D *perp_inclz = new TH1D("perp_inclz", "perp_inclz", 7,z_edge);

	TH1D *perpeta = new TH1D("perpeta", "perpeta", 50, -2.5, 2.5);
	TH1D *jeteta = new TH1D("jeteta", "jeteta", 50, -2.5, 2.5);


	//TFile *f = new TFile("outfile_pp5TeV.root", "read");
	TFile *f = new TFile("outfile_pp5TeV_set22_grp000_00000.root", "read");
	...
	for (int ien=0; ien<nent; ien++)
	{
    	...
    	for (int kjet = 0; kjet < i_njet; kjet++)
    	{
				...
				if (f_jet_pt[kjet]<20 || f_jet_pt[kjet]>40) continue;
